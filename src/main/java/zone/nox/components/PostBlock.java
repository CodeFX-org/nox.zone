...
public record PostBlock(Post post) implements Component {
	...
	public Element compose() {
		return div.classes(STYLE.block).children(
				a.classes(STYLE.index).href(post.slug()).text("#%03d".formatted(post.index())),
				a.classes(STYLE.title).href(post.slug()).text(post.title()),
				span.classes(STYLE.date).text(format(post.date())),
				a.classes(STYLE.more).href(post.slug()).text("More >>")
		);
	}
